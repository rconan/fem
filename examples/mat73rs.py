...
for input in fem_inputs:
    ...
    for k in range(n_in):
        ...
        d['properties']['nodeID'] = fem_properties['nodeID'].astype(
            np.uint32).flatten().tolist()
        try:
            d['properties']['csNumber'] = fem_properties['csNumber'].astype(
                np.uint32).flatten().tolist()
        except:
            pass
    ...
    for k in range(n_out):
        ...
        for l in ["types", "descriptions", "indices"]:
            ...
            if l in ["types", "descriptions"]:
                d[l] = bytes(d[l]).decode()
            if l in ["indices"]:
                d[l] = fem_outputs[input][k][l].flatten().astype(
                    np.uint32).tolist()
        ...
        try:
            ...
            pass
        try:
            d['properties']['component'] = fem_properties['component'].astype(
                np.int32).flatten().tolist()
        except:
            pass
        try:
            d['properties']['csLabel'] = bytes(
                d['properties']['csLabel']).decode()
        except:
            pass
        out_dict.append(d)
